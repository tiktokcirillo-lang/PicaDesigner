...
interface Scenario {terraUsage: UsageProfile; solUsage: UsageProfile; terraCalls: number; solRate: number; imageCostUsd:number;imageRate:number}
const SCENARIOS: Record<Exclude<ScenarioName, 'REAL_CALIBRATED'>, Scenario> = {
  LOW: {terraUsage: {inputTokens: 4500, cachedInputTokens: 1000, cacheWriteTokens: 300, outputTokens: 900}, solUsage: {inputTokens: 6000, cachedInputTokens: 1500, cacheWriteTokens: 400, outputTokens: 900}, terraCalls: 3, solRate: 0.05,imageCostUsd:.08,imageRate:.35},
  NORMAL: {terraUsage: {inputTokens: 9000, cachedInputTokens: 1800, cacheWriteTokens: 700, outputTokens: 1800}, solUsage: {inputTokens: 10000, cachedInputTokens: 2500, cacheWriteTokens: 800, outputTokens: 1800}, terraCalls: 5, solRate: 0.20,imageCostUsd:.16,imageRate:.6},
  HIGH: {terraUsage: {inputTokens: 15000, cachedInputTokens: 2200, cacheWriteTokens: 1200, outputTokens: 3000}, solUsage: {inputTokens: 14000, cachedInputTokens: 3000, cacheWriteTokens: 1400, outputTokens: 2800}, terraCalls: 5, solRate: 0.40,imageCostUsd:.30,imageRate:.85},
};
...
const calibratedScenario = (calibration: CostCalibrationFile): Scenario => {
  ...
  return {terraUsage: average(terraCalls), solUsage: average(solCalls), terraCalls: terraCalls.length, solRate: calibration.solExecuted ? 1 : 0,imageCostUsd:0,imageRate:0};
};
export const simulateAICost = (scenario: ScenarioName, projects = 20, calibration?: CostCalibrationFile) => {
  ...
  const costs = Array.from({length: projects}, (_, index) => terra + (index < Math.round(projects * settings.solRate) ? sol : 0)+(index<Math.round(projects*settings.imageRate)?settings.imageCostUsd:0));
  return {scenario, projects, averageProjectCost: costs.reduce((sum, value) => sum + value, 0) / projects, monthlyCost: costs.reduce((sum, value) => sum + value, 0), terraCalls: settings.terraCalls * projects, solEscalationPercentage: Math.round(settings.solRate * 100),imageGenerationPercentage:Math.round(settings.imageRate*100),imageCalls:Math.round(projects*settings.imageRate),costDistribution: {min: Math.min(...costs), max: Math.max(...costs)}, projectsOverTarget: costs.filter((value) => value > 0.5).length, projectsOverHardLimit: costs.filter((value) => value > 0.75).length};
};
