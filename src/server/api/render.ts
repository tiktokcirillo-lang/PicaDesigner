...
type SourceAwareRenderRequest = CreateRenderSessionRequest & {
  sourceAssetIds?: string[];
};
const withoutPrivateSourceRefs = (session: RenderSession): RenderSession =>
  JSON.parse(
...
export const createRenderRouter = () => {
  ...
  router.post("/session", async (req, res) => {
    try {
      const body = req.body as SourceAwareRenderRequest,
        ...
        session = createRenderSession({
          ...body,
          assetRegistry: source.registry,
        });
      return res.json(withoutPrivateSourceRefs(session));
    } catch (e) {
      return res
        .status(422)
        .json({
          error: e instanceof Error ? e.message : "Render planning failed.",
        });
    }
  });
  return router;
