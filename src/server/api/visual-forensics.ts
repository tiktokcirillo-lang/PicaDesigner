...
export const createVisualForensicsRouter = (): Router => {
  ...
  router.post('/analyze', async (request, response) => {
    try {
      const body = request.body as Partial<VisualForensicsInput> & {projectId?: string; sourceAssetId?:string; imageMetadata?: Omit<ReferenceSourceMetadata, 'mediaType'>};
      if ((!body.image&&!body.sourceAssetId) || !body.projectId) return response.status(400).json({error: 'sourceAssetId and projectId are required'});
      if (!/^[A-Za-z0-9_-]{1,128}$/.test(body.projectId)) return response.status(400).json({error: 'projectId is invalid'});
      let image=body.image,imageMetadata=body.imageMetadata;
      if(body.sourceAssetId){
        const asset=await applicationProjectSourceAssetRepository.get(body.projectId,body.sourceAssetId);
        if(!asset||asset.role!=="visual_reference")return response.status(404).json({error:"Visual reference is unavailable."});
        const effective=await reconcileProjectSourceAsset(asset,applicationProjectSourceAssetStore),bytes=effective.status==="available"?await applicationProjectSourceAssetStore.get(asset.backingRef,body.projectId):undefined;
        if(!bytes)return response.status(409).json({error:"O arquivo original da referência não está mais disponível."});
        image={kind:"base64",data:Buffer.from(bytes).toString("base64"),mediaType:asset.mediaType};
        imageMetadata={fileName:asset.originalFilename,fileSize:asset.byteSize,width:asset.width,height:asset.height,aspectRatio:asset.aspectRatio,imageFingerprint:asset.checksum};
      }
      if(!image)return response.status(400).json({error:"Visual reference is required."});
      const config = loadOpenAIConfig();
      const provider = new OpenAIProvider(config);
      const session = await createReferenceIntelligence({image, projectId: body.projectId, imageMetadata, analysisDepth: body.analysisDepth, semanticExclusions: body.semanticExclusions, context: body.context}, {
