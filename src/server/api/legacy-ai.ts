...
export const createLegacyAIRouter = (): Router => {
  ...
  router.post("/generate-design-spec", async (request, response) => {
    try {
      const body = request.body as Partial<GenerateDesignSpecRequest> & {
        prompt?: string;
        image?: VisualInput;
      };
      ...
      if (body.prompt) {
        if (body.prompt.length > 50_000)
          return response
            .status(400)
            .json({ error: "legacy prompt must be at most 50,000 characters" });
        return response.json(
      ...
      if (!body.destinationTool?.trim() || body.destinationTool.length > 120)
        return response
          .status(400)
          .json({ error: "destinationTool is required" });
      if (!body.tone?.trim() || body.tone.length > 200)
        return response.status(400).json({ error: "tone is required" });
      return response.json(
        await generateDesignSpec(
          {
            projectId,
            copy: body.copy,
            format: body.format,
            formatContext: body.formatContext,
            destinationTool: body.destinationTool,
            tone: body.tone,
            brandInput: body.brandInput,
            brandIntelligence: body.brandIntelligence,
            referenceIntelligence: body.referenceIntelligence,
            adaptedDesignConstraints: body.adaptedDesignConstraints,
            creativeDirection: body.creativeDirection,
            layoutIntelligence: body.layoutIntelligence,
            artDirectorReview: body.artDirectorReview,
