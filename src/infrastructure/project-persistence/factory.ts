...
import { NeonProjectPersistenceRepository } from "./neon-repository.js";
class UnavailableProjectRepository {
  private fail(): never {
    throw new PersistenceUnavailableError(
      "Durable project persistence unavailable.",
    );
  }
  createProject() {
    return this.fail();
...
export const createProjectRepository = (
  ...
  if (env.DATABASE_URL)
    return new NeonProjectPersistenceRepository(env.DATABASE_URL);
  const production = env.NODE_ENV === "production";
  if (
    !production &&
    (env.PROJECT_ALLOW_INMEMORY_PERSISTENCE ?? "true").toLowerCase() === "true"
  )
    return new MockDurableProjectRepository(createMockDurableDatabase());
  return new UnavailableProjectRepository() as ProjectPersistenceRepository;
};
export const applicationProjectRepository = createProjectRepository();
