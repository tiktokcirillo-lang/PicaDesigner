...
const imageDataUrl = (input: VisualInput, maxMb: number): string => {
  if (input.kind === 'url') throw new UnsupportedAIInputError('Remote image URLs are not accepted; provide base64 or bytes to prevent SSRF.');
  if (!ALLOWED_MEDIA.has(input.mediaType)) throw new UnsupportedAIInputError(`Unsupported image media type: ${input.mediaType}.`);
  const base64 = input.kind === 'bytes' ? Buffer.from(input.data).toString('base64') : input.data.replace(/^data:[^;]+;base64,/, '');
  const estimatedBytes = Math.ceil(base64.length * 0.75);
  if (estimatedBytes > maxMb * 1024 * 1024) throw new UnsupportedAIInputError(`Image exceeds the ${maxMb} MB server limit.`);
  return `data:${input.mediaType};base64,${base64}`;
};

const mapError = (error: unknown): AIProviderError => {
  if (error instanceof OpenAI.AuthenticationError) return new AIAuthenticationError('OpenAI authentication failed.', error);
  if (error instanceof OpenAI.RateLimitError) return new AIRateLimitError('OpenAI rate limit reached.', error);
  if (error instanceof OpenAI.APIConnectionTimeoutError) return new AITimeoutError('OpenAI request timed out.', error);
  if (error instanceof OpenAI.APIError) return new AIProviderError(`OpenAI request failed with status ${error.status ?? 'unknown'}.`, error);
  return new AIProviderError('OpenAI request failed.', error);
};
const retryable = (error: unknown): boolean => error instanceof OpenAI.RateLimitError || error instanceof OpenAI.APIConnectionTimeoutError || error instanceof OpenAI.APIConnectionError || (error instanceof OpenAI.APIError && TRANSIENT_STATUS.has(error.status ?? 0));

export class OpenAIProvider implements AIProvider {
  readonly id = 'openai' as const;
  private readonly client: OpenAI;
  constructor(private readonly config: OpenAIConfig, client?: OpenAI) {this.client = client ?? createOpenAIClient(config);}
