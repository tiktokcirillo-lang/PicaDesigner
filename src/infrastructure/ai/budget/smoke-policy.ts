...
export const resolveSmokeMaxCost = (requested?: number): number => {
  ...
  if (!Number.isFinite(value) || value <= 0 || value > ABSOLUTE_SMOKE_MAX_COST_USD) throw new AIBudgetExceededError(`Smoke max cost must be greater than 0 and no more than ${ABSOLUTE_SMOKE_MAX_COST_USD.toFixed(2)} USD.`);
  return value;
};
