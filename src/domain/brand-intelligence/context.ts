...
export const buildBrandDesignContext = (brand: BrandDNA, compatibility: BrandReferenceCompatibilityReport, adapted: AdaptedDesignConstraints) => ({
  ...
  hardConstraints: brand.hardConstraints.map(({id,domain,rule,severity,authority}) => ({id,domain,rule,severity,authority})),
  prohibitedBehaviors:brand.prohibitedBehaviors.map(({id,domain,rule,severity,authority})=>({id,domain,rule,severity,authority})),
