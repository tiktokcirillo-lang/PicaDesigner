...
import {ANTI_CLICHE_KNOWLEDGE,CREATIVE_PROMPT_POLICY,buildMessageHierarchy,type CommunicationBrief,type CreativeDirectionRequest} from '../../domain/creative-direction/index.js';
...
export const buildCreativePromptModules = (brief: CommunicationBrief, request: CreativeDirectionRequest, contexts: {brand?: unknown; reference?: unknown; adapted?: unknown}) => ({
  ...
  approvedMessageHierarchy:buildMessageHierarchy(brief),
  ...
  outputContract: 'Return concise structured data only. Preserve approved copy. Use only supplied IDs in all *Refs arrays. claimRefsUsed may reference explicit approved message IDs only. Use null for an unnecessary tension or metaphor. Declare every possible violation, invented claim, literal reference transfer, and brand-drift trait in its audit array. Do not generate coordinates or image prompts.',
