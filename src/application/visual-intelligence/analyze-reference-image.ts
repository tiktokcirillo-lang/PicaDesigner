...
export const analyzeReferenceImage = async (request: AnalyzeReferenceImageRequest, dependencies: AnalyzeDependencies): Promise<VisualIntelligenceResult> => {
  ...
  return {forensics, designDNA, quality: {...quality, ...(critic ? {critic} : {})}, aiUsage: {inputTokens: ledger.inputTokens, cachedInputTokens: ledger.cachedInputTokens, outputTokens: ledger.outputTokens, totalCostUsd: ledger.totalCostUsd, targetCostUsd: config.targetProjectCostUsd, limitCostUsd: config.maxProjectCostUsd, modelsUsed: [...new Set(ledger.modelCalls.map(({model}) => model))], calls: ledger.modelCalls, escalationStatus, escalationReason: decision.reasons.join(' ') || undefined, ...(budget.status === 'blocked' ? {escalationStatus: 'budget_blocked'} : {})}};
};
